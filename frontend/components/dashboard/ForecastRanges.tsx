...
export default function ForecastRanges({ forecast }: Props) {
  ...
  return (
    <div className="aurum-card p-4 flex flex-col gap-3">
            ...
            <div key={label} className="flex items-center gap-3">
              ...
              <div className="flex-1 flex flex-col gap-0.5">
                <div className="flex justify-between text-xs">
                  <span className="text-[var(--text-muted)]">${lo.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
                  <span className={bias > 0 ? 'text-[#22c55e]' : bias < 0 ? 'text-[#ef4444]' : 'text-[var(--text-muted)]'}>
                    {bias > 0 ? '+' : ''}{bias.toFixed(1)}%
                  </span>
                  <span className="text-[var(--text-muted)]">${hi.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
                </div>
      ...
      <div className="border-t border-[var(--border-subtle)] pt-2 flex justify-between text-xs text-[var(--text-muted)]">
